...
- **FR-003**: 각 태그는 1자 이상 20자 이하이며, 알파벳·숫자·한글·하이픈(`-`)·언더스코어(`_`)만 허용한다. 공백·특수문자를 포함하는 태그나 20자 초과 태그는 오류로 처리한다.
- **FR-004**: 중복 태그가 지정된 경우 자동으로 중복을 제거하고 저장한다.
- **FR-005**: 태그 없이 Todo를 생성하는 기존 흐름은 변경 없이 유지되어야 한다.
- **FR-006**: `todo list --tag <태그명>` 옵션으로 해당 태그를 가진 항목만 필터링해 조회할 수 있어야 한다. `--tag`는 한 번에 하나의 태그만 지정할 수 있다(복수 태그 필터는 v1 범위 외).
- **FR-007**: `--tag` 필터는 기존 `--filter`(상태) 및 `--priority` 필터와 조합하여 사용할 수 있어야 한다.
- **FR-008**: 목록 출력 시 각 항목의 태그 정보를 기존 행 끝에 `tags=[work, urgent]` 형태로 함께 표시해야 한다. 태그가 없는 항목은 `tags=[]`로 표시한다.
...
- **TodoItem (확장)**: 기존 TodoItem에 `tags_json` TEXT 컬럼이 추가된다. 태그 목록(0~5개)은 JSON 배열로 직렬화하여 단일 컬럼에 저장하며, Python `tags` property(getter/setter)로 접근한다. 별도 Tag 엔티티 또는 관계 테이블은 사용하지 않는다.
...
- **SC-006**: US1·US2의 Acceptance Scenarios를 모두 통과하는 자동화 테스트가 존재하며, 전체 커버리지 90% 이상을 유지한다.
...
- Q: 태그 저장 방식(별도 테이블 vs. 직렬화)? → A: `todos` 테이블의 `tags_json` TEXT 컬럼에 JSON으로 저장 (단순함 우선, 추가 외부 패키지 없음, 별도 테이블 불필요)
- Q: 목록 출력 시 태그 표시 형식은? → A: 기존 행 끝에 `tags=[work, urgent]` 형태로 추가, 태그 없으면 `tags=[]`
- Q: `--tag` 필터를 여러 개 동시에 지정할 수 있는가? → A: 단일 태그 필터만 허용, 복수 태그 필터는 v1 범위 외
- Q: 태그 허용 문자 범위는? → A: 알파벳·숫자·한글·하이픈(`-`)·언더스코어(`_`)만 허용, 공백 및 특수문자 불허
- Q: 기존 DB에 태그 테이블 없으면? → A: 앱 시작 시 `create_all`로 자동 생성, 기존 항목은 태그 없음으로 유지 (별도 마이그레이션 도구 미사용)
- Q: 태그 기능 추가 후 테스트 커버리지 임계치는? → A: 기존과 동일하게 90% 유지

## Assumptions

- 태그는 알파벳·숫자·한글·하이픈(`-`)·언더스코어(`_`)만 허용하며, 공백 및 기타 특수문자는 오류로 처리한다.
- 태그 수정(edit) 기능은 이번 기능 범위 밖이다. Todo 항목 자체의 수정 기능도 v1 범위 외다.
- 태그는 `todos` 테이블의 `tags_json` TEXT 컬럼(JSON 직렬화)에 저장한다. 별도 테이블을 추가하지 않는다. 앱 시작 시 `create_all` + `_migrate_add_tags_column()`으로 신규·기존 DB를 모두 처리하며, 기존 항목은 `tags_json='[]'`로 유지된다. 별도 마이그레이션 도구는 사용하지 않는다.
